    ...
    async getWebsocketURL() {
        ...
        return new Promise<string>((resolve, reject) => {
            ...
            this._server = http.createServer(async (req, res) => {
                ...
                const proxiedHeaders = new Headers(req.headers as any)
            ...
            this._wss.on('connection', async (ws, request) => {
                try {
                    ...
                    const address = this._server!.address() as AddressInfo
                    exec(`"${riotClientPath}" --client-config-url="http://${address.address}:${address.port}" --launch-product=valorant --launch-patchline=live`)
