...
async function sleuth(file: string): Promise<Partial<LogInfo>> {
  ...
  for await (const line of rl) {
    ...
    if (info.clientVersion === undefined) {
      ...
      if (match) {
        info.clientVersion = match[1]; // No longer adding "shipping-", ShooterGame.log has it now
      }
    }
...
export default async function logSleuth(): Promise<
  Partial<LogInfo> | undefined
> {
